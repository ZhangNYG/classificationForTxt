...
def get_data(client,filename):
    with client.read(filename,encoding='utf-8') as f:
        ...
        for line in f:
            ...
            print("\n data_tmp_batch ___iii",len(data_tmp_batch),"\n")
            if counter != 0 and counter % 32 ==0:
                # 生成batch
                result_batch = []
                label_batch = []
                # label_np_batch = np.zeros([32, 3])
                for i_batch in range(32):
                    print("\n data_tmp_batch",len(data_tmp_batch),"\n")
                    if len(data_tmp_batch[i_batch]) == 2 :
                        if len(data_tmp_batch[i_batch][-1]) <= 4 :
                            result = javaInstance.run(data_tmp_batch[i_batch][0])
                            if len(result) != 128:
                                continue
                            ...
                            tmp_lable = [0,0,0]
                            tmp_lable[index_np] = 1
                            label_batch.append(tmp_lable)
