...
def mode_base():
    ...
    regula_loss = tf.add_n([ tf.nn.l2_loss(v) for v in vars
                if 'biases' not in v.name ]) * 0.001
    ...
    loss = tf.reduce_mean(losses + regula_loss)
    ...
    while True:
        ...
        for file_loop in range(3):
            ...
            _, train_loss = sess.run([train_step, loss],
                                     feed_dict={x_input: x_batch, y_lable: y_batch})
            all_STEP += 1
