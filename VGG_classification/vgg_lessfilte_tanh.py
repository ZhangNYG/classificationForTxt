...
def mode_base():
    ...
    with tf.name_scope('conv1_1') as scope:
        kernel = tf.Variable(tf.truncated_normal([1, 3, 1, 16], dtype=tf.float32,
    ...
    with tf.name_scope('conv1_2') as scope:
        kernel = tf.Variable(tf.truncated_normal([1, 3, 16, 16], dtype=tf.float32,
                                                 stddev=1e-1), name='weights')
    ...
    with tf.name_scope('conv2_1') as scope:
        kernel = tf.Variable(tf.truncated_normal([1, 3, 16, 32], dtype=tf.float32,
                                                 stddev=1e-1), name='weights')
    ...
    with tf.name_scope('conv2_2') as scope:
        kernel = tf.Variable(tf.truncated_normal([1, 3, 32, 32], dtype=tf.float32,
                                                 stddev=1e-1), name='weights')
    ...
    with tf.name_scope('conv3_1') as scope:
        kernel = tf.Variable(tf.truncated_normal([1, 3, 32, 64], dtype=tf.float32,
                                                 stddev=1e-1), name='weights')
    ...
    hiddenLayer1 = add_layer("layer1", pool5_flat, in_size=shape, out_size=1024, activation_function=tf.tanh)

    hiddenLayer2 = add_layer("layer2", hiddenLayer1, in_size=1024, out_size=512, activation_function=tf.tanh)

    hiddenLayer3 = add_layer("layer3", hiddenLayer2, in_size=512, out_size=256, activation_function=tf.tanh)

    hiddenLayer4 = add_layer("layer4", hiddenLayer3, in_size=256, out_size=128, activation_function=tf.tanh)
    ...
    sess = tf.Session()
    sess.run(init)
    return x_input, y_lable, prediction, loss, train_step, sess, pool5_flat
        ...
        for line in f:
            ...
            if len(data_tmp) == 2:
                if len(data_tmp[-1]) <= 4:
                    ...
                    result_np = result_np * 100
                    prediction_val = sess.run(prediction, feed_dict={x_input: result_np})
                    prediction_list = prediction_val.tolist()
                    print(prediction_list)
                    pool5_flat_val =  sess.run(pool5_flat , feed_dict={x_input: result_np})
                    # print(pool5_flat_val)
                    # print(len(pool5_flat_val[0]))
                    # print(type(pool5_flat_val))
                    ...
                    print(list_index)
                    print("准确率   ：", true_counter / float(counter_test))
...
if __name__ == '__main__':
    ...
    # 获取模型
    x_input, y_lable, prediction, loss, train_step, sess ,pool5_flat = mode_base()

    epoch_times = 0
    while True:
        epoch_times += 1
        fileListForloop = fileList[0:1]
