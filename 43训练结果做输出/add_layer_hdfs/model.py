...
class BiRNN(object):
    ...
    def __init__(self, embedding_size, rnn_size, layer_size,
        ...
        fc_w1 = tf.Variable(tf.truncated_normal([2 * rnn_size, 2 * rnn_size], stddev=0.1), name='fc_w1')
        fc_b1 = tf.Variable(tf.zeros([2 * rnn_size]), name='fc_b1')
        ...
        fc_w2 = tf.Variable(tf.truncated_normal([2 * rnn_size, rnn_size], stddev=0.1), name='fc_w2')
        fc_b2 = tf.Variable(tf.zeros([rnn_size]), name='fc_b2')
        ...
        fc_w3 = tf.Variable(tf.truncated_normal([rnn_size, n_classes], stddev=0.1), name='fc_w3')
        fc_b3 = tf.Variable(tf.zeros([n_classes]), name='fc_b3')
        ...
        self.prob = tf.nn.softmax(self.logits3)
        self.probResult = tf.argmax(self.prob, axis=1)
        self.cost = tf.losses.softmax_cross_entropy(self.targets, self.logits3)
